...
const AutomationCenter: React.FC = () => {
  ...
  const [contacts, setContacts] = useState<IGContact[]>([
    ...
    { id: 'c5', username: 'amit_clicks', category: 'general', avatar: 'https://picsum.photos/seed/amit/100/100' },
  ]);
  ...
  const [historyItems] = useState<IGAutomationItem[]>([
    ...
    { id: 'h3', type: 'dm', user: 'amit_clicks', category: 'general', status: 'completed', timestamp: '3h ago', url: 'https://www.instagram.com/direct/' },
  ...
  const handleDeleteContact = (id: string) => {
    if (confirm("Remove this contact from your automated list?")) {
  ...
  const getCategoryBadge = (cat: ContactCategory) => {
    switch (cat) {
      case 'business': return <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider flex items-center gap-1"><Briefcase size={10}/> Business</span>;
      case 'friends': return <span className="bg-pink-100 text-pink-700 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider flex items-center gap-1"><Users size={10}/> Friend</span>;
      default: return <span className="bg-slate-100 text-slate-700 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider flex items-center gap-1"><UserPlus size={10}/> General</span>;
    }
  };
        ...
        <div className="flex gap-3 w-full md:w-auto">
          {activeTab === 'history' && (
            <select 
              ...
              className="px-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:ring-2 focus:ring-pink-500 shadow-sm"
            >
          ...
          {activeTab === 'contacts' && (
            <button 
              onClick={() => alert("Search for new Instagram accounts to follow/track...")}
              className="p-3 bg-pink-600 text-white rounded-2xl shadow-lg hover:bg-pink-700 transition-all active:scale-95"
            >
              <Plus size={20} />
            </button>
          )}
            ...
            <tbody className="divide-y divide-slate-50">
              {items.length > 0 ? items.map((item) => (
                <tr key={item.id} className="group hover:bg-slate-50/50 transition-all cursor-pointer">
                  ...
                  <td className="px-8 py-6"><div className="flex items-center gap-3 mb-2"><img src={`https://picsum.photos/seed/${item.user}/50/50`} className="w-9 h-9 rounded-full border shadow-sm" alt={item.user} /><span className="font-bold text-slate-800">@{item.user}</span></div>{getCategoryBadge(item.category)}</td>
          ...
          <div className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredContacts.length > 0 ? filteredContacts.map((contact) => (
                <div key={contact.id} className="bg-slate-50 rounded-3xl p-6 border border-slate-100 hover:shadow-md transition-all group relative">
                  <div className="flex items-center gap-4 mb-4">
                    <img src={contact.avatar} className="w-14 h-14 rounded-full border-2 border-white shadow-sm" alt={contact.username} />
                    <div>
                      <p className="font-black text-slate-800">@{contact.username}</p>
                      {getCategoryBadge(contact.category)}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => alert("Tracking insights for " + contact.username)} className="flex-1 py-2 bg-white text-slate-700 text-xs font-black rounded-xl border border-slate-200 hover:bg-slate-50 transition-colors">Insights</button>
                    <button onClick={() => handleDeleteContact(contact.id)} className="p-2 bg-white text-red-500 rounded-xl border border-slate-200 hover:bg-red-50 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={16} /></button>
                  </div>
                </div>
              )) : (
                <div className="col-span-full text-center py-20 text-slate-400 font-bold">No contacts found</div>
              )}
            </div>
          </div>
        )}
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredHistory.length > 0 ? filteredHistory.map((h) => (
                <tr key={h.id} className="hover:bg-slate-50/50 transition-all">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-slate-800 text-sm">@{h.user}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                ...
                <tr><td colSpan={5} className="text-center py-20 text-slate-400 font-bold">No history available</td></tr>
              )}
            </tbody>
          </table>
        )}
